...
# Pedro Lima (FARMER3_* key): a real CAR property (Nova Mamoré/RO) but a key the company never onboarded. Not allow-listed → the buyer refuses him.
if [ -n "${FARMER3_PRIVATE_KEY:-}" ]; then
  (cd apps/farmer-agent && FARMER_NAME="Pedro Lima" FARMER_PRIVATE_KEY=$FARMER3_PRIVATE_KEY FARMER_PORT=4022 FARMER_PUBLIC_URL=http://localhost:4022 FARMER_SAMPLES=5BA735B8 nohup npx tsx src/index.ts > ../../.logs/farmer3.log 2>&1 < /dev/null &)
fi

...
- **Who gets paid**: the CAR registry is public, so anyone could compute a true-looking proof for someone else's farm. The buyer only pays suppliers it onboarded: the company allow-lists a supplier's address in its `AgentWallet` (`setPayee`) after contract/KYC, the buyer agent marks other sellers `knownSupplier: false` and flags the same CAR offered by two keys (`carConflict`). The demo includes an impostor farmer agent for exactly this: one farmer (three registered properties) plus an impostor offering one of them under an unknown key. Next: a gov.br-signed wallet↔CAR-holder binding anchored on-chain, or a zkTLS proof of the SICAR login.

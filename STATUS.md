...
## FEITO 12/09 ~11:00
- **Lote com vários produtores**: João Silva (porta 4020) com os 3 imóveis (…9C62, …4C8E, …0A28); Maria Souza opcional (FARMER2_SAMPLES). `./scripts/dev-up.sh` sobe os dois. Buyer LLM recebe a lista de agentes.
...
- **Impostor na demo**: Pedro Lima (porta 4022, chave FARMER3, com gás, NÃO cadastrado) tem o CAR real …5BA7 (Nova Mamoré/RO, 16.435 ha, zip baixado por você). Agente marca UNKNOWN + CAR-CONFLICT e recusa. Comando da demo tem 2 URLs (4020 João, 4022 Pedro).
...
- Demo: http://localhost:4030/demo (dois cliques). Lado FARM: dropdown João Silva (3 propriedades) / Pedro Lima (1 propriedade real). Lado TRADER: checkbox por propriedade dentro de cada fornecedor; lado TRADER lista fornecedores por nome com checkbox. Ledger de vendas zerado.
